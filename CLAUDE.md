...
--verbose`). Results are persisted to `~/.ostack-dev/evals/` with auto-comparison
...
├── bin/             # CLI utilities (ostack-repo-mode, ostack-slug, ostack-config, etc.)
...
2. **If missing, AskUserQuestion** — let the user tell you or let ostack search the repo
3. **Persist the answer to CLAUDE.md** so we never have to ask again

This applies to test commands, eval commands, deploy commands, and any other
project-specific behavior. The project owns its config; ostack reads it.
...
When developing ostack, `.claude/skills/ostack` may be a symlink back to this
working directory (gitignored). This means skill changes are **live immediately** —
great for rapid iteration, risky during big refactors where half-written skills
could break other Claude Code sessions using ostack concurrently.

**Check once per session:** Run `ls -la .claude/skills/ostack` to see if it's a
symlink or a real copy. If it's a symlink to your working directory, be aware that:
- Template changes + `bun run gen:skill-docs` immediately affect all ostack invocations
- Breaking changes to SKILL.md.tmpl files can break concurrent ostack sessions
- During large refactors, remove the symlink (`rm .claude/skills/ostack`) so the
  global install at `~/.claude/skills/ostack/` is used instead

**For plan reviews:** When reviewing plans that modify skill templates or the
gen-skill-docs pipeline, consider whether the changes should be tested in isolation
before going live (especially if the user is actively using ostack in other windows).
...
When estimating or discussing effort, always show both human-team and CC+ostack time:
...
Contributors can store long-range vision docs and design documents in `~/.ostack-dev/plans/`.
...
The active skill lives at `~/.claude/skills/ostack/`. After making changes:

1. Push your branch
2. Fetch and reset in the skill directory: `cd ~/.claude/skills/ostack && git fetch origin && git reset --hard origin/main`
3. Rebuild: `cd ~/.claude/skills/ostack && bun run build`

Or copy the binary directly: `cp browse/dist/browse ~/.claude/skills/ostack/browse/dist/browse`
