...
describeIfSelected('Ship workflow E2E', ['ship-local-workflow'], () => {
  ...
  beforeAll(() => {
    shipRemoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ostack-ship-remote-'));
    shipWorkDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ostack-ship-work-'));
...
describeIfSelected('ostack-upgrade E2E', ['ostack-upgrade-happy-path'], () => {
  ...
  testConcurrentIfSelected('ostack-upgrade-happy-path', async () => {
    const mockOstack = path.join(upgradeDir, '.claude', 'skills', 'ostack');
    const result = await runSkillTest({
...
IMPORTANT: The install directory is at ./.claude/skills/ostack — use that exact path.`,
    ...
    recordE2E(evalCollector, '/ostack-upgrade happy path', 'ostack-upgrade E2E', result, {
