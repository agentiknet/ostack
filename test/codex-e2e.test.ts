...
describeCodex('Codex E2E', () => {
  ...
  testIfSelected('codex-discover-skill', async () => {
    // Install ostack-review skill to a temp HOME and ask Codex to list skills
    const skillDir = path.join(ROOT, '.agents', 'skills', 'ostack-review');
    ...
    expect(
      outputLower.includes('review') || outputLower.includes('ostack') || outputLower.includes('skill'),
    ).toBe(true);
  }, 120_000);

  // Validates that Codex can invoke the ostack-review skill, run a diff-based
  ...
  testIfSelected('codex-review-findings', async () => {
    // Install ostack-review skill and ask Codex to review the current repo
    const skillDir = path.join(ROOT, '.agents', 'skills', 'ostack-review');

    const result = await runCodexSkill({
      skillDir,
      prompt: 'Run the ostack-review skill on this repository. Review the current branch diff and report your findings.',
      timeoutMs: 540_000,
      cwd: ROOT,
      skillName: 'ostack-review',
    });
