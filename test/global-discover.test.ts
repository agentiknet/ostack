...
const scriptPath = join(import.meta.dir, "..", "bin", "ostack-global-discover.ts");

describe("ostack-global-discover", () => {
  describe("normalizeRemoteUrl", () => {
    // Dynamically import to test the exported function
    let normalizeRemoteUrl: (url: string) => string;

    beforeEach(async () => {
      const mod = await import("../bin/ostack-global-discover.ts");
    ...
    test("SSH and HTTPS for same repo normalize to same URL", () => {
      const ssh = normalizeRemoteUrl("git@github.com:garrytan/ostack.git");
      const https = normalizeRemoteUrl("https://github.com/garrytan/ostack.git");
      const httpsNoDotGit = normalizeRemoteUrl("https://github.com/garrytan/ostack");
