...
const OSTACK_DEV_DIR = path.join(os.homedir(), '.ostack-dev');
const HEARTBEAT_PATH = path.join(OSTACK_DEV_DIR, 'e2e-live.json');
...
export async function runSkillTest(options: {
  ...
  if (runId) {
    try {
      runDir = path.join(OSTACK_DEV_DIR, 'e2e-runs', runId);
  ...
  if (browseErrors.length > 0 || exitReason !== 'success') {
    try {
      const failureDir = runDir || path.join(workingDirectory, '.ostack', 'test-transcripts');
