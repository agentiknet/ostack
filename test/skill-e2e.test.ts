...
// Eval result collector — accumulates test results, writes to ~/.ostack-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.ostack', 'test-transcripts');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/ostack/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with ostack installed globally, the fallback path
    // ~/.claude/skills/ostack/browse/dist/browse exists, so we get READY.
...
OVERRIDE: Write contributor logs to ${logsDir}/ instead of ~/.ostack/contributor-logs/

Now try this browse command (it will fail — there is no binary at this path):
/nonexistent/path/browse goto https://example.com

This is a ostack issue (the browse binary is missing/misconfigured).
File a contributor report about this issue. Then tell me what you filed.`,
    ...
    expect(logContent).toContain('Hey ostack team');
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a ostack skill. The session preamble detected _SESSIONS=4 (the user has 4 ostack windows open).
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.ostack/') && !l.includes('qa-reports/'),
    );
...
describeIfSelected('ostack-upgrade E2E', ['ostack-upgrade-happy-path'], () => {
  let upgradeDir: string;
  let remoteDir: string;

  beforeAll(() => {
    upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-upgrade-'));
    remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ostack-remote-'));
    ...
    // Create mock ostack install directory (local-git type)
    const mockOstack = path.join(upgradeDir, '.claude', 'skills', 'ostack');
    fs.mkdirSync(mockOstack, { recursive: true });

    // Init as a git repo
    run('git', ['init'], mockOstack);
    run('git', ['config', 'user.email', 'test@test.com'], mockOstack);
    run('git', ['config', 'user.name', 'Test'], mockOstack);

    // Create bare remote
    run('git', ['init', '--bare'], remoteDir);
    run('git', ['remote', 'add', 'origin', remoteDir], mockOstack);

    // Write old version files
    fs.writeFileSync(path.join(mockOstack, 'VERSION'), '0.5.0\n');
    fs.writeFileSync(path.join(mockOstack, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    fs.writeFileSync(path.join(mockOstack, 'setup'),
      '#!/bin/bash\necho "Setup completed"\n', { mode: 0o755 });

    // Initial commit + push
    run('git', ['add', '.'], mockOstack);
    run('git', ['commit', '-m', 'initial'], mockOstack);
    run('git', ['push', '-u', 'origin', 'HEAD:main'], mockOstack);

    // Create new version (simulate upstream release)
    fs.writeFileSync(path.join(mockOstack, 'VERSION'), '0.6.0\n');
    fs.writeFileSync(path.join(mockOstack, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.6.0 — 2026-03-15\n\n- New feature: interactive design review\n- Fix: snapshot flag validation\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    run('git', ['add', '.'], mockOstack);
    run('git', ['commit', '-m', 'release 0.6.0'], mockOstack);
    run('git', ['push', 'origin', 'HEAD:main'], mockOstack);

    // Reset working copy back to old version
    run('git', ['reset', '--hard', 'HEAD~1'], mockOstack);

    // Copy ostack-upgrade skill
    fs.mkdirSync(path.join(upgradeDir, 'ostack-upgrade'), { recursive: true });
    fs.copyFileSync(
      path.join(ROOT, 'ostack-upgrade', 'SKILL.md'),
      path.join(upgradeDir, 'ostack-upgrade', 'SKILL.md'),
    );
  ...
  testIfSelected('ostack-upgrade-happy-path', async () => {
    const mockOstack = path.join(upgradeDir, '.claude', 'skills', 'ostack');
    const result = await runSkillTest({
      prompt: `Read ostack-upgrade/SKILL.md for the upgrade workflow.

You are running /ostack-upgrade standalone. The ostack installation is at ./.claude/skills/ostack (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.claude/skills/ostack — use that exact path.`,
      workingDirectory: upgradeDir,
      maxTurns: 20,
      timeout: 180_000,
      testName: 'ostack-upgrade-happy-path',
      runId,
    });

    logCost('/ostack-upgrade happy path', result);

    // Check that the version was updated
    const versionAfter = fs.readFileSync(path.join(mockOstack, 'VERSION'), 'utf-8').trim();
    const output = result.output || '';
    ...
    recordE2E('/ostack-upgrade happy path', 'ostack-upgrade E2E', result, {
