 ...
 * Used by skill:check and local freshness checks.
 */
...
function loadObrainContext(relativePath: string): string {
  const obrainRoot = path.resolve(ROOT, 'obrain');
  const filePath = path.resolve(obrainRoot, relativePath);
...
const HOST_PATHS: Record<Host, HostPaths> = {
  claude: {
    skillRoot: '~/.claude/skills/ostack',
    localSkillRoot: '.claude/skills/ostack',
    binDir: '~/.claude/skills/ostack/bin',
    browseDir: '~/.claude/skills/ostack/browse/dist',
  },
  codex: {
    skillRoot: '$OSTACK_ROOT',
    localSkillRoot: '.agents/skills/ostack',
    binDir: '$OSTACK_BIN',
    browseDir: '$OSTACK_BROWSE',
  },
};
...
/** ostack's 10 AI slop anti-patterns — shared between DESIGN_METHODOLOGY and DESIGN_HARD_RULES */
...
OSTACK_ROOT="$HOME/.codex/skills/ostack"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.agents/skills/ostack" ] && OSTACK_ROOT="$_ROOT/.agents/skills/ostack"
OSTACK_BIN="$OSTACK_ROOT/bin"
OSTACK_BROWSE="$OSTACK_ROOT/browse/dist"
`
    : '';
...
\`\`\`bash
${runtimeRoot}_UPD=$(${ctx.paths.binDir}/ostack-update-check 2>/dev/null || ${ctx.paths.localSkillRoot}/bin/ostack-update-check 2>/dev/null || true)
...
_CONTRIB=$(${ctx.paths.binDir}/ostack-config get ostack_contributor 2>/dev/null || true)
_PROACTIVE=$(${ctx.paths.binDir}/ostack-config get proactive 2>/dev/null || echo "true")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
echo "PROACTIVE: $_PROACTIVE"
source <(${ctx.paths.binDir}/ostack-repo-mode 2>/dev/null) || true
...
function generateUpgradeCheck(ctx: TemplateContext): string {
  return `If \`PROACTIVE\` is \`"false"\`, do not proactively suggest ostack skills — only invoke
them when the user explicitly asks. The user opted out of proactive suggestions.

If output shows \`UPGRADE_AVAILABLE <old> <new>\`: read \`${ctx.paths.skillRoot}/ostack-upgrade/SKILL.md\` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If \`JUST_UPGRADED <from> <to>\`: tell user "Running ostack v{to} (just updated!)" and continue.`;
}
...
function generateCompletenessSection(): string {
  return `## Conviction and Completeness — Often Wrong, Never in Doubt
...
- Create a GitHub issue assigned to that person:
  ...
  gh issue create \\
    ...
    --body "Found failing on branch <current-branch>. Failure is pre-existing.\\n\\n**Error:**\\n\`\`\`\\n<first 10 lines>\\n\`\`\`\\n\\n**Last modified by:** <author>\\n**Noticed by:** ostack /ship on <date>" \\
...
Check if the diff touches frontend files using \`ostack-diff-scope\`:

\`\`\`bash
source <(${ctx.paths.binDir}/ostack-diff-scope <base> 2>/dev/null)
\`\`\`
...
\`\`\`bash
${ctx.paths.binDir}/ostack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
\`\`\`
...
**Local:** \`.ostack/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md\`

**Project-scoped:**
\`\`\`bash
eval "$(~/.claude/skills/ostack/bin/ostack-slug 2>/dev/null)" && mkdir -p ~/.ostack/projects/$SLUG
\`\`\`
Write to: \`~/.ostack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md\`
...
[ -f .ostack/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
\`\`\`
...
If user picks H → write \`.ostack/no-test-bootstrap\` and continue without tests.
...
If user picks C → write \`.ostack/no-test-bootstrap\`. Tell user: "If you change your mind later, delete \`.ostack/no-test-bootstrap\` and re-run." Continue without tests.
...
Write to \`~/.ostack/projects/{slug}/{user}-{branch}-eng-review-test-plan-{datetime}.md\`:
...
Write to \`~/.ostack/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md\`:
...
\`~/.claude/skills/ostack/${first}/SKILL.md\`
...
which codex 2>/dev/null && echo "CODEX_AVAILABLE" || echo "CODEX_NOT_AVAILABLE"
# Respect old opt-out
OLD_CFG=$(~/.claude/skills/ostack/bin/ostack-config get codex_reviews 2>/dev/null || true)
...
\`\`\`bash
~/.claude/skills/ostack/bin/ostack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"medium","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
...
\`\`\`bash
~/.claude/skills/ostack/bin/ostack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"large","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
...
**Log the result:**
\`\`\`bash
${ctx.paths.binDir}/ostack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
Replace STATUS with "clean" or "issues_found", SOURCE with "codex+subagent", "codex-only", "subagent-only", or "unavailable".`;
}

// ─── Design Hard Rules (OpenAI framework + ostack slop blacklist) ───
...
Source: [OpenAI "Designing Delightful Frontends with GPT-5.4"](https://developers.openai.com/blog/designing-delightful-frontends-with-gpt-5-4) (Mar 2026) + ostack design methodology.`;
}

function generateSlugEval(ctx: TemplateContext): string {
  return `eval "$(${ctx.paths.binDir}/ostack-slug 2>/dev/null)"`;
}

function generateSlugSetup(ctx: TemplateContext): string {
  return `eval "$(${ctx.paths.binDir}/ostack-slug 2>/dev/null)" && mkdir -p ~/.ostack/projects/$SLUG`;
}
...
function codexSkillName(skillDir: string): string {
  if (skillDir === '.' || skillDir === '') return 'ostack';
  // Don't double-prefix: ostack-upgrade → ostack-upgrade (not ostack-ostack-upgrade)
  if (skillDir.startsWith('ostack-')) return skillDir;
  return `ostack-${skillDir}`;
}
...
function processTemplate(tmplPath: string, host: Host = 'claude'): { outputPath: string; content: string } {
  ...
  // Replace {{OBRAIN:path}} placeholders (resolve from obrain directory)
  let content = tmplContent.replace(/\{\{OBRAIN:([^}]+)\}\}/g, (_match, relativePath: string) => {
    return loadObrainContext(relativePath.trim());
  });
  ...
  if (host === 'codex') {
    ...
    content = content.replace(/~\/\.claude\/skills\/ostack/g, ctx.paths.skillRoot);
    content = content.replace(/\.claude\/skills\/ostack/g, ctx.paths.localSkillRoot);
    content = content.replace(/\.claude\/skills\/review/g, '.agents/skills/ostack/review');
