# Contributing to ostack

Thanks for wanting to make ostack better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

ostack skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/ostack/` (your global install). But when you're developing ostack itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone <repo> && cd ostack
...
Contributor mode turns ostack into a self-improving tool. Enable it and Claude Code
will periodically reflect on its ostack experience — rating it 0-10 at the end of
each major workflow step. When something isn't a 10, it thinks about why and files
a report to `~/.ostack/contributor-logs/` with what happened, repro steps, and what
would make it better.

```bash
~/.claude/skills/ostack/bin/ostack-config set ostack_contributor true
```

The logs are for **you**. When something bugs you enough to fix, the report is
already written. Fork ostack, symlink your fork into the project where you hit
the issue, fix it, and open a PR.

### The contributor workflow

1. **Use ostack normally** — contributor mode reflects and logs issues automatically
2. **Check your logs:** `ls ~/.ostack/contributor-logs/`
3. **Fork and clone ostack** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where ostack annoyed you)
   ln -sfn /path/to/your/ostack-fork .claude/skills/ostack
   cd .claude/skills/ostack && bun install && bun run build
   ```
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using ostack** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix ostack while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ ostack sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on ostack inside the ostack repo

When you're editing ostack skills and want to test them by actually using ostack
...
```
ostack/                          <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── ostack -> ../../         <- symlink back to repo root
│   ├── review -> ostack/review
│   ├── ship -> ostack/ship
...
When E2E tests run, they produce machine-readable artifacts in `~/.ostack-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.ostack-dev/` for post-mortem debugging and trend analysis.
...
ostack generates SKILL.md files for two hosts: **Claude** (`.claude/skills/`) and **Codex** (`.agents/skills/`). Every template change needs to be generated for both.
...
| Aspect | Claude | Codex |
|--------|--------|-------|
| Output directory | `{skill}/SKILL.md` | `.agents/skills/ostack-{skill}/SKILL.md` (generated at setup, gitignored) |
| Frontmatter | Full (name, description, allowed-tools, hooks, version) | Minimal (name + description only) |
| Paths | `~/.claude/skills/ostack` | `$OSTACK_ROOT` (`.agents/skills/ostack` in a repo, otherwise `~/.codex/skills/ostack`) |
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/ostack`. `bin/dev-teardown` restores the global one.
...
**This is the recommended way to develop ostack.** Symlink your ostack checkout
into the project where you actually use it, so your changes are live while you
do real work:

```bash
# In your core project
ln -sfn /path/to/your/ostack-checkout .claude/skills/ostack
cd .claude/skills/ostack && bun install && bun run build
```

Now every ostack skill invocation in this project uses your working tree. Edit a
...
```bash
rm .claude/skills/ostack
```

Claude Code falls back to `~/.claude/skills/ostack/` automatically.
