...
This document explains **why** ostack is built the way it is. For setup and commands, see CLAUDE.md. For contributing, see CONTRIBUTING.md.
...
1. **Compiled binaries.** `bun build --compile` produces a single ~58MB executable. No `node_modules` at runtime, no `npx`, no PATH configuration. The binary just runs. This matters because ostack installs into `~/.claude/skills/` where users don't expect to manage a Node.js project.
...
The server writes `.ostack/browse.json` (atomic write via tmp + rename, mode 0o600):
...
Cookies are the most sensitive data ostack handles. The design:

1. **Keychain access requires user approval.** First cookie import per browser triggers a macOS Keychain dialog. The user must click "Allow" or "Always Allow." ostack never silently accesses credentials.
...
3. **Database is read-only.** ostack copies the Chromium cookie DB to a temp file (to avoid SQLite lock conflicts with the running browser) and opens it read-only. It never modifies your real browser's cookie database.
...
```
Browser events → CircularBuffer (in-memory) → Async flush to .ostack/*.log
```
...
1. **Update check** — calls `ostack-update-check`, reports if an upgrade is available.
...
```bash
jq '.tests[] | select(.exit_reason == "timeout") | .last_tool_call' ~/.ostack-dev/evals/_partial-e2e.json
```
...
`eval:compare` diffs two eval runs. `eval:summary` aggregates stats across all runs in `~/.ostack-dev/evals/`.
